...
module ``07: Strings and Conversions`` =
    ...
    let ``02 Getting a substring (Part 1).`` () =
        let a = "bright" 
        // a.[start...end] where you can have either or both
    ...
    let ``07 Make an int conversion fail`` () =
        ...
        (fun () ->
            ...
            //so because a is trying to become a int while it is a float
            // this will then cause the exception error to occur
    ...
    *)
    ...
    // use %A when you are putting in multiple types within brackets.
