...
module ``08: Putting the Function into Functional Programming`` = 
    ...
    let ``10 A function can return a function (Part 1).`` () =
        ...
        let j = i 10
        let k = j 9
        // J is a function as well, so remember to substitute the k as well
        k |> should equal 1
    ...
    let ``12 You can write a function as a one-liner (Part 1).`` () =
        (fun hardon -> fun softon -> hardon - softon) 10 9 |> should equal 1

    [<Test>]
    let ``13 You can write a function as a one-liner (Part 2).`` () =
        (fun hardon softon -> hardon - softon) 10 9 |> should equal 1
    ...
    let ``16 A function is executed when it is called, NOT when it is defined or referenced (Part 2).`` () =
        (fun () ->
            let f a =
                failwith "An exception will be thrown as soon as this is executed."
                a + 2
            // The function is only executed in its lexical scope 
    ...
    let ``19 Partially specifying arguments (Part 3).`` () =
        ...
        let f animal noise = animal + " says " + noise
        let cows x = f "cow" (x + ", de gozaru")  // <-- multiple words on this line, or you may want to make this a multi-line thing.  You MUST use `f`.
    ...
    *)
    ...
    let ``26 Functions have types`` () =
        ...
        // Because of the way function chaining works, the function below type will the 
        // remainder of the parameters not filled and the output
    ...
    *)
    ...
    let ``28 Type annotations for function types`` () =
        ...
        a "sk" "ipping" |> should equal "skipping"
